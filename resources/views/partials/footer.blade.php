<!-- Footer -->
<footer class="bg-slate-950 text-gray-400 py-16 md:py-24 border-t-8 border-primary dark:border-primary/80">
    <div class="container mx-auto px-4 md:px-12">
        <div class="flex flex-col lg:flex-row justify-between items-start gap-12 md:gap-16">
                ...
                <div class="flex justify-center md:justify-start space-x-6 text-2xl md:text-3xl">
                    @if(isset($socialMedia))
                        @foreach($socialMedia as $social)
                            <a href="{{ $social->url }}" target="_blank" class="text-gray-500 hover:text-primary transition-colors" title="{{ $social->platform }}">
            ...
            <div class="w-full lg:w-1/4 text-center md:text-left">
                <h5 class="text-white font-black uppercase tracking-widest mb-6 md:mb-8 border-b-2 border-primary inline-block">{{ __('messages.quick_links') }}</h5>
                <ul class="space-y-3 md:space-y-4 font-bold text-xs md:text-sm">
                    <li><a href="{{ url('/') }}" class="hover:text-primary transition-colors uppercase">{{ __('messages.home') }}</a></li>
                    <li><a href="{{ route('landing.profile') }}" class="hover:text-primary transition-colors uppercase">{{ __('messages.profile') }}</a></li>
                    <li><a href="{{ route('landing.news') }}" class="hover:text-primary transition-colors uppercase">{{ __('messages.news') }}</a></li>
                    <li><a href="{{ url('/#prodi') }}" class="hover:text-primary transition-colors uppercase">{{ __('messages.study_programs') }}</a></li>
                    <li><a href="{{ url('/#kontak') }}" class="hover:text-primary transition-colors uppercase">{{ __('messages.contact_us') }}</a></li>
                </ul>
            </div>
            <div class="w-full lg:w-1/3 text-center md:text-left">
                <h5 class="text-white font-black uppercase tracking-widest mb-6 md:mb-8 border-b-2 border-primary inline-block">{{ __('messages.contact_us') }}</h5>
                <ul class="space-y-4 md:space-y-6 text-sm md:text-base">
                    <li class="flex flex-col md:flex-row items-center md:items-start">
                        <i class="fas fa-map-marker-alt mb-2 md:mt-1.5 md:mr-4 text-primary"></i>
