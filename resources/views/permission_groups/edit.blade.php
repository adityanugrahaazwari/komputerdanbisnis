...
<div class="bg-white dark:bg-slate-900 rounded-[2rem] shadow-xl p-8 md:p-10 max-w-2xl mx-auto border border-gray-100 dark:border-slate-800">
    <form action="{{ route('permission-groups.update', $permissionGroup->id) }}" method="POST" class="space-y-8">
        ...
        <div>
            ...
            <input type="text" name="name" value="{{ old('name', $permissionGroup->name) }}" class="w-full px-4 py-4 bg-gray-50 dark:bg-slate-800 border border-gray-100 dark:border-slate-700 rounded-2xl focus:ring-2 focus:ring-red-600 outline-none transition-all dark:text-white font-medium">
        </div>
