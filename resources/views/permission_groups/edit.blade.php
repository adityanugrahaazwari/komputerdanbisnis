...
@section('header', 'Edit Permission Group: ' . $permissionGroup->name)
...
<div class="bg-white dark:bg-slate-900 rounded-[2rem] shadow-xl p-8 md:p-10 max-w-2xl mx-auto border border-gray-100 dark:border-slate-800">
    <form action="{{ route('permission-groups.update', $permissionGroup->id) }}" method="POST" class="space-y-8">
        @csrf
        @method('PUT')
        ...
        <div>
            ...
            <input type="text" name="name" value="{{ old('name', $permissionGroup->name) }}" class="w-full px-4 py-4 bg-gray-50 dark:bg-slate-800 border border-gray-100 dark:border-slate-700 rounded-2xl focus:ring-2 focus:ring-red-600 outline-none transition-all dark:text-white font-medium" required>
        </div>
        ...
        <div class="flex items-center justify-between pt-8 border-t border-gray-100 dark:border-slate-800">
            ...
            <button type="submit" class="bg-red-700 hover:bg-red-800 text-white font-black px-10 py-4 rounded-2xl shadow-xl shadow-red-200 dark:shadow-none uppercase tracking-widest text-sm transition transform active:scale-95">
                <i class="fas fa-save mr-2"></i> Perbarui Grup
            </button>
        </div>
    </form>
</div>
