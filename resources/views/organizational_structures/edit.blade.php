...
<div class="bg-white dark:bg-slate-900 rounded-[2rem] shadow-xl p-8 md:p-10 max-w-2xl mx-auto border border-gray-100 dark:border-slate-800">
    <form action="{{ route('organizational-structures.update', $organizationalStructure->id) }}" method="POST" enctype="multipart/form-data" class="space-y-8">
        ...
        <div>
            ...
            <input type="text" name="name" value="{{ old('name', $organizationalStructure->name) }}" class="w-full px-4 py-4 bg-gray-50 dark:bg-slate-800 border border-gray-100 dark:border-slate-700 rounded-2xl focus:ring-2 focus:ring-red-600 outline-none transition-all dark:text-white font-medium" required>
        </div>

        <div>
            <label class="block text-gray-700 dark:text-gray-300 text-xs font-black uppercase tracking-widest mb-3 ml-1">Jabatan</label>
            <input type="text" name="position" value="{{ old('position', $organizationalStructure->position) }}" class="w-full px-4 py-4 bg-gray-50 dark:bg-slate-800 border border-gray-100 dark:border-slate-700 rounded-2xl focus:ring-2 focus:ring-red-600 outline-none transition-all dark:text-white font-medium" required>
        </div>
        ...
        <div class="grid grid-cols-2 gap-6">
            ...
            <div>
                ...
                @if($organizationalStructure->image)
                    <p class="text-[10px] mt-2 text-gray-500 italic uppercase">Ada foto terunggah. Biarkan kosong jika tidak ingin mengubah.</p>
                @endif
            </div>
        </div>
        ...
        <div class="flex items-center justify-between pt-8 border-t border-gray-100 dark:border-slate-800">
            ...
            <button type="submit" class="bg-red-700 hover:bg-red-800 text-white font-black px-10 py-4 rounded-2xl shadow-xl shadow-red-200 dark:shadow-none uppercase tracking-widest text-sm transition transform active:scale-95">
                <i class="fas fa-save mr-2"></i> Perbarui Struktur
            </button>
        </div>
    </form>
</div>
