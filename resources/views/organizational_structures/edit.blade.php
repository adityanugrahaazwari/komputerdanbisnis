...
@section('header', 'Edit Structure Element: ' . $organizationalStructure->name)
...
<div class="bg-white dark:bg-slate-900 rounded-[2rem] shadow-xl p-8 md:p-10 max-w-2xl mx-auto border border-gray-100 dark:border-slate-800">
    <form action="{{ route('organizational-structures.update', $organizationalStructure->id) }}" method="POST" enctype="multipart/form-data" class="space-y-8">
        @csrf
        @method('PUT')
        ...
        <div>
            ...
            <select name="parent_id" class="w-full px-4 py-4 bg-gray-50 dark:bg-slate-800 border border-gray-100 dark:border-slate-700 rounded-2xl focus:ring-2 focus:ring-red-600 outline-none transition-all dark:text-white font-medium">
                ...
                @foreach($parents as $parent)
                    <option value="{{ $parent->id }}" {{ $organizationalStructure->parent_id == $parent->id ? 'selected' : '' }}>{{ $parent->position }} - {{ $parent->name }}</option>
                @endforeach
            </select>
        </div>
        ...
        <div>
            ...
            <input type="text" name="name" value="{{ old('name', $organizationalStructure->name) }}" class="w-full px-4 py-4 bg-gray-50 dark:bg-slate-800 border border-gray-100 dark:border-slate-700 rounded-2xl focus:ring-2 focus:ring-red-600 outline-none transition-all dark:text-white font-medium">
        </div>

        <div>
            <label class="block text-gray-700 dark:text-gray-300 text-xs font-black uppercase tracking-widest mb-3 ml-1">Jabatan</label>
            <input type="text" name="position" value="{{ old('position', $organizationalStructure->position) }}" class="w-full px-4 py-4 bg-gray-50 dark:bg-slate-800 border border-gray-100 dark:border-slate-700 rounded-2xl focus:ring-2 focus:ring-red-600 outline-none transition-all dark:text-white font-medium">
        </div>
        ...
        <div class="grid grid-cols-2 gap-6">
            <div>
                ...
                <input type="number" name="order" value="{{ old('order', $organizationalStructure->order) }}" class="w-full px-4 py-4 bg-gray-50 dark:bg-slate-800 border border-gray-100 dark:border-slate-700 rounded-2xl focus:ring-2 focus:ring-red-600 outline-none transition-all dark:text-white font-medium">
            </div>
            <div>
                ...
                <p class="text-[10px] text-gray-400 mt-2 italic ml-1">Rekomendasi ukuran: 3:4 (Contoh: 600x800px). @if($organizationalStructure->image) Kosongkan jika tidak ingin mengubah. @endif</p>
            </div>
        </div>
